...
8. **[Synergy](https://symless.com/synergy/)**(macOS/Windows/Linux)
...
16. **[eZip](https://ezip.awehunt.com/)**(macOS）
...
21. **[滴答清单](https://dida365.com/)**(macOS/Windows/Android/iOS/Chrome App）
...
29. **[frp](https://github.com/fatedier/frp)**(macOS/Windows/Linux)
      ...
      内网穿透
      安全提醒：穿透远程桌面或是ssh时，请修改默认端口，并启用更强的安全措施（修改默认端口/启用点对点内网穿透/使用密钥而非密码登录），防止被穿透电脑被黑（血泪教训）
...
3. **简悦（SimpleRead）**
   ...
   个人使用举例：网页直接转markdown存储到坚果云，自动同步到本地，永久收藏有价格文章原始数据。

4. **Dark Reader**

      黑色主题，适用于任何网站。关爱眼睛，就使用Dark Reader进行夜间和日间浏览。

5. **扩展管理器（Extension Manager）**
...
13. **Web Server for Chrome**

      快速搭建本地Web服务器(当然，你也可以`python -m http.server`，实测传输大文件速度不如python命令)
